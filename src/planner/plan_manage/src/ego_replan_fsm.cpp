...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    for (int i = 0; i < 8; i++)
    {
      nh.param("global_goal/relative_pos_" + to_string(i) + "/x", swarm_relative_pts_[0][i][0], -1.0);
      nh.param("global_goal/relative_pos_" + to_string(i) + "/y", swarm_relative_pts_[0][i][1], -1.0);
      nh.param("global_goal/relative_pos_" + to_string(i) + "/z", swarm_relative_pts_[0][i][2], -1.0);
      nh.param("global_goal1/relative_pos_" + to_string(i) + "/x", swarm_relative_pts_[1][i][0], -1.0);
      nh.param("global_goal1/relative_pos_" + to_string(i) + "/y", swarm_relative_pts_[1][i][1], -1.0);
      nh.param("global_goal1/relative_pos_" + to_string(i) + "/z", swarm_relative_pts_[1][i][2], -1.0);
      nh.param("global_goal2/relative_pos_" + to_string(i) + "/x", swarm_relative_pts_[2][i][0], -1.0);
      nh.param("global_goal2/relative_pos_" + to_string(i) + "/y", swarm_relative_pts_[2][i][1], -1.0);
      nh.param("global_goal2/relative_pos_" + to_string(i) + "/z", swarm_relative_pts_[2][i][2], -1.0);
      nh.param("global_goal3/relative_pos_" + to_string(i) + "/x", swarm_relative_pts_[3][i][0], -1.0);
      nh.param("global_goal3/relative_pos_" + to_string(i) + "/y", swarm_relative_pts_[3][i][1], -1.0);
      nh.param("global_goal3/relative_pos_" + to_string(i) + "/z", swarm_relative_pts_[3][i][2], -1.0);
    }
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case EXEC_TRAJ:
    {
      ...
      // local target close to waypoints wps_
      if (current_wp_ < wps_.size() && (wps_[current_wp_] - local_target_pt_).norm() < 1.0)
      {
        current_wp_++;
        // use printf with color
        printf("\033[47;30m\n[drone %d reached waypoint %d]==============================================\033[0m\n",
               planner_manager_->pp_.drone_id, current_wp_);
        // changeFSMExecState(WAIT_OTHERS, "FSM");
  ...
  void EGOReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state, string pos_call)
  {
    ...
    static string state_str[8] = {"INIT", "WAIT_TARGET", "GEN_NEW_TRAJ", "REPLAN_TRAJ", "EXEC_TRAJ", "EMERGENCY_STOP", "SEQUENTIAL_START", "WAIT_OTHERS"};
  ...
  void EGOReplanFSM::formationWaypointCallback(const geometry_msgs::PoseStampedPtr &msg)
  {
    ...
    relative_pos << swarm_relative_pts_[end_times_][id][0],
                    swarm_relative_pts_[end_times_][id][1],
                    swarm_relative_pts_[end_times_][id][2];
    end_times_ = (end_times_ + 1) % 4;
